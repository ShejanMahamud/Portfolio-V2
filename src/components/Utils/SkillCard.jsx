const SkillCard = ({ title, description, level, icon }) => {
  return (
    <div className='flex items-center justify-start h-20 rounded-xl text-gray-800 dark:text-zinc-300 bg-gradient-to-l from-white to-zinc-50 dark:bg-gradient-to-l dark:from-stone-900 dark:to-neutral-900 ring-1 ring-stone-200 dark:ring-stone-800 select-none group'>
      <div className='-mt-20 ml-6 bg-zinc-50 dark:bg-neutral-900 ring-1 ring-stone-200 dark:ring-stone-800 rounded-xl rotate-45 duration-200 p-3 text-2xl backdrop-blur-lg flex items-center justify-center'>
        <div className="group-hover:scale-125 group-hover:duration-300 -rotate-45">
        {icon}
        </div>
      </div>
      <div className="p-4 w-full">
        <h3 className="text-md font-semibold text-zinc-700 dark:text-zinc-300">{title}</h3>
        <div className="grid grid-cols-5 gap-2 items-center">
          <div className="col-span-3">
            <p className="text-zinc-600 dark:text-zinc-400 truncate text-xs" title={description}>{description}</p>
          </div>
          <div className="col-span-2">
            <div className="flex space-x-0.5" title={level}>
              ...
              {[...Array(5 - level).keys()].map((index) => (
                <div key={index} className="h-1.5 w-4 bg-stone-300 dark:bg-stone-700"></div>
              ))}
            </div>
          </div>
        </div>
